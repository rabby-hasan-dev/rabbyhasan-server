...
const ProjectSchema = z.object({
    body: z.object({
        ...
        githubRepoClient: z.string().url("Invalid URL for GitHub client repository").min(1, "GitHub client repository is required"), // Required string with URL validation
        githubRepoServer: z.string().url("Invalid URL for GitHub server repository").min(1, "GitHub server repository is required"), // Required string with URL validation
...
const UpdateProjectSchema = z.object({
    body: z.object({
        ...
        githubRepoClient: z.string().url("Invalid URL for GitHub client repository").min(1, "GitHub client repository is required").optional(),
        githubRepoServer: z.string().url("Invalid URL for GitHub server repository").min(1, "GitHub server repository is required").optional(),
